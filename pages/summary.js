...
export default function SummaryPage() {
  ...
  const [certificates] = useState([
    { name: 'Data Structure and Algorithm', completed: true, date: '2024-01-15' },
  ...
  const getRarityColor = (type) => {
    switch (type) {
      case 'Common': return 'text-slate-700 bg-gradient-to-r from-slate-100 to-gray-200 border border-slate-300';
      case 'Rare': return 'text-blue-700 bg-gradient-to-r from-blue-100 to-sky-200 border border-blue-300';
      case 'Epic': return 'text-purple-700 bg-gradient-to-r from-purple-100 to-violet-200 border border-purple-300';
      case 'Legendary': return 'text-amber-700 bg-gradient-to-r from-yellow-100 to-amber-200 border border-yellow-300';
      default: return 'text-slate-700 bg-gradient-to-r from-slate-100 to-gray-200 border border-slate-300';
    }
  };

  // Calculate pie chart paths
  const circumference = 2 * Math.PI * 45;
  const attendedOffset = circumference - (attendanceData.attended / 100) * circumference;

  return (
    <>
      <Head>
        <title>Student Progress Summary</title>
        <meta name="description" content="Track your academic progress and achievements" />
      </Head>
          ...
          <div className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-2xl shadow-xl px-6 py-3">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <div className="text-lg font-bold text-gray-900">{tokenData.currentBalance.toLocaleString()}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Track your learning journey, attendance records, and achievement milestones in one comprehensive dashboard.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-sky-200 hover:shadow-3xl transition-all duration-300">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">📅 Attendance Rate</h2>
                ...
                <div className="relative w-48 h-48">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-blue-900">{attendanceData.attended}%</div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 text-center">
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 rounded-xl p-4 border border-green-200">
                  <div className="text-lg font-bold text-green-700">{attendanceData.attendedClasses}</div>
                ...
                <div className="bg-gradient-to-br from-red-50 to-rose-100 rounded-xl p-4 border border-red-200">
                  <div className="text-lg font-bold text-red-700">{attendanceData.totalClasses - attendanceData.attendedClasses}</div>
            ...
            {/* Token Accumulation */}
            <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-sky-200 hover:shadow-3xl transition-all duration-300">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">💰 Token Statistics</h2>
                ...
                <div className="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-xl p-4 border border-yellow-200">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xl font-bold text-gray-900">{tokenData.totalEarned.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-xl p-4 border border-red-200">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xl font-bold text-gray-900">{tokenData.totalSpent.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xl font-bold text-gray-900">{tokenData.currentBalance.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-sky-200 hover:shadow-3xl transition-all duration-300 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">🎓 Course Certificates</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {certificates.map((cert, index) => (
                <div key={index} className="bg-gradient-to-br from-gray-50 to-slate-100 rounded-xl p-6 border shadow-sm">
                  ...
                  {cert.completed ? (
                    <div className="text-sm text-gray-600">
                      <span className="font-medium">Completed:</span> {formatDate(cert.date)}
                    </div>
                  ) : (
                    <div>
          ...
          {/* NFT Collection */}
          <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-sky-200 hover:shadow-3xl transition-all duration-300">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 text-center">🖼️ NFT Collection</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {ownedNFTs.map((nft) => (
                <div key={nft.id} className="bg-gradient-to-br from-gray-50 to-slate-100 rounded-2xl overflow-hidden border-2 border-sky-200 shadow-lg hover:shadow-xl transition-all duration-300">
                  <div className={`${nft.color} p-6 relative overflow-hidden`}>
                    ...
                    <div className="absolute top-4 right-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${getRarityColor(nft.type)}`}>
                        {nft.type}
                      </span>
                    </div>
                  </div>
            ...
            {ownedNFTs.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">No NFTs collected yet. Start attending classes to earn rewards!</p>
              </div>
            )}
          </div>
        </div>
